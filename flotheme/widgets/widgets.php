 ...
 * Register Sidebars
 */
function flo_init_sidebars() {
	foreach (flotheme_get_sidebars() as $id => $title) {
		register_sidebar(
				array(
					'id' => $id,
					'name' => __($title, 'flotheme'),
					'description' => __($title, 'flotheme'),
...
add_action('init', 'flo_init_sidebars');

/**
 * Register Theme Widgets 
 */
function flo_init_widgets() {
	register_widget('Flotheme_Blog_Widget');
	register_widget('Flotheme_Tweets_Widget');
	register_widget('Flotheme_Flickr_Widget');
	register_widget('Flotheme_About_Widget');
}

add_action('widgets_init', 'flo_init_widgets');

/**
 * Image field for widgets 
 */
class Flotheme_WidgetImageField 
{
	private $image_id;
	private $src;
	private $width;
	...
	function __construct($widget = null, $image_id = 0) {
		...
		if ($uri && in_array($file, array('widgets.php')) && is_admin()) {
			...
			wp_enqueue_script('widgetimagefield', FLOTHEME_URL . '/assets/js/widgets.js', array('jquery', 'jquery-ui-core', 'thickbox', 'media-upload'), false, true);
